...
ESX.RegisterServerCallback('esx_securitas:getFineList', function(source, cb, category)
	MySQL.Async.fetchAll('SELECT * FROM fine_types_secu WHERE category = @category', {
		['@category'] = category
	}, function(fines)
		cb(fines)
	end)
...
AddEventHandler('esx_securitas', function(resource)
...
RegisterServerEvent('esx_securitas:requestarrest')
AddEventHandler('esx_securitas:requestarrest', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_securitas:getarrested', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_securitas:doarrested', _source)
end)

RegisterServerEvent('esx_securitas:requestrelease')
AddEventHandler('esx_securitas:requestrelease', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_securitas:getuncuffed', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_securitas:douncuffing', _source)
end)
